...
void simpleConditionPractice(int lower, int upper)
{
    ...
    if (lower < upper)
        printf("Lower is less than upper (one line).\n");
    else
        printf("Lower is not less than upper\n");
}

void complexConditionPractice(int lower, int upper)
{
    // if with logical operatiors
    if (lower < upper && lower > 0)
    {
        printf("Lower is less than upper and greater than zero\n");
    }
    else if (lower > upper || lower < 0)
    {
        printf("Lower is either greater than upper or less than zero\n");
    }
    else
    {
        printf("Lower is equal to upper and greater than or equal to zero\n");
    }

    // jist using int values
    if (lower - upper)
    {
